...
|---|--:|---|
| ✅ Done | 152 | core behavior implemented + tested, or wired to real data |
| 🟡 Partial | 185 | engine/logic done with headline UI wired, or faithful screen awaiting full binding |
...
wired to real data**. The remaining **185 Partial** stories fall into exactly two honest buckets:
...
_Last updated: after correcting AUTH-01/AUTH-02's stale "no guest demo" premise now that `/demo` is a real, no-login guest topic (Lecture reader + conflict resolve + pipeline run), on top of the public certificate verify page at `/verify/[code]` (TEST-11), linking the guest demo path from Login (HOME-12), a password-confirmed change-email action (SETTINGS-03), feeding missed seeded error-drills into the Gap Map (GAP-07), and the blind-spot subsystem (ANALYTICS-07/REVIEW-06/REVIEW-14) — 152 Done; remainder is field-polish or deferred-with-justification._
